...
export const generateBio = async (role: string, experience: string, goals: string) => {
  try {
    const prompt = `You are an expert executive career coach specializing in the 2026 tech landscape.
...
export const enhanceProjectDescription = async (description: string) => {
  try {
    const prompt = `You are a high-level product marketing strategist. 
